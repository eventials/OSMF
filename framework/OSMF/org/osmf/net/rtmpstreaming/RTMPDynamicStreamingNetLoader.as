...
package org.osmf.net.rtmpstreaming
{
	...
	public class RTMPDynamicStreamingNetLoader extends NetLoader
	{
		...
		override protected function createNetStreamSwitchManager(connection:NetConnection, netStream:NetStream, dsResource:DynamicStreamingResource):NetStreamSwitchManagerBase
		{
			...
			return new NetStreamSwitchManager(connection, netStream, dsResource, metrics, getDefaultSwitchingRules(metrics, dsResource.switchRules));
		}
		...
		private function getDefaultSwitchingRules(metrics:RTMPNetStreamMetrics, hashRule:Object):Vector.<SwitchingRuleBase>
		{
			var rules:Vector.<SwitchingRuleBase> = new Vector.<SwitchingRuleBase>();

			for(var hash:String in hashRule)
			{
				if (hash == 'SufficientBandwidthRule')
				{
					var bandwidthSafetyMultiple:Number = hashRule['SufficientBandwidthRule']['bandwidthSafetyMultiple'];
					var minDroppedFps:int = hashRule['SufficientBandwidthRule']['minDroppedFps'];
					rules.push(new SufficientBandwidthRule(metrics, bandwidthSafetyMultiple, minDroppedFps));
				}
				else if (hash == 'InsufficientBufferRule')
				{
					var minBufferLength:Number = hashRule['InsufficientBufferRule']['minBufferLength'];
					rules.push(new InsufficientBufferRule(metrics, minBufferLength));
				}
				else if (hash == 'DroppedFramesRule')
				{
					var downSwitchByOne:int = hashRule['DroppedFramesRule']['downSwitchByOne'];
					var downSwitchByTwo:int = hashRule['DroppedFramesRule']['downSwitchByTwo'];
					var downSwitchToZero:int = hashRule['DroppedFramesRule']['downSwitchToZero'];
					rules.push(new DroppedFramesRule(metrics, downSwitchByOne, downSwitchByTwo, downSwitchToZero));
				}
				else if (hash == 'InsufficientBandwidthRule')
				{
					var bitrateMultiplier:Number = hashRule['InsufficientBandwidthRule']['bitrateMultiplier'];
					rules.push(new InsufficientBandwidthRule(metrics, bitrateMultiplier));
				}
			}
